...
const fallbackEmpty = ({ message }) => <div className="flex flex-col items-center justify-center py-16 text-[var(--color-muted)]"><p className="mt-4 text-sm">{message}</p></div>;
const fallbackBadge = ({ children }) => <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-[var(--color-bg)] text-[var(--color-muted)]">{children}</span>;
      ...
      <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4 md:p-5 shadow-sm mb-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-[var(--color-muted)] mt-1">هذه الصفحة تعرض تقارير مبنية على الحركات المرتبطة بالدفتر (meta).</p>
          </div>
      ...
      {!activeId ? (
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4 md:p-5 shadow-sm">
            ...
            <p className="text-sm text-[var(--color-muted)] mb-3">عرض تقرير شهري للدفتر مع إمكانية تصدير PDF ومشاركته.</p>
          ...
          <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4 md:p-5 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={exportLedgerPDF} disabled={pdfExporting} className="px-3 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 disabled:opacity-50" aria-label="تصدير كشف حساب PDF">{pdfExporting ? 'جاري…' : 'كشف حساب PDF'}</button>
                <button type="button" onClick={exportLedgerTxCSV} className="px-3 py-2 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text)] text-sm font-medium hover:bg-[var(--color-bg)]" aria-label="تصدير CSV">تصدير CSV</button>
              </div>
            </div>
            ...
            <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="p-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)]">
                ...
                <div className="mt-1 text-xl font-bold text-[var(--color-text)]">{(txs || []).length}</div>
              </div>
              <div className="p-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)]">
                <div className="text-xs text-[var(--color-muted)]">صافي آخر 30 يوم</div>
                <div className="mt-1 text-xl font-bold text-[var(--color-text)]"><Currency value={pl30?.net || 0} /></div>
              </div>
              <div className="p-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)]">
                <div className="text-xs text-[var(--color-muted)]">صافي آخر سنة</div>
                <div className="mt-1 text-xl font-bold text-[var(--color-text)]"><Currency value={pl365?.net ?? 0} /></div>
              </div>
              <div className="p-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)]">
            ...
            <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4 md:p-5 shadow-sm">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-2">
                  <span className="text-2xl font-bold text-[var(--color-text)]">{compliance.pct ?? 0}%</span>
                ...
                {compliance.completionPct != null && (
                  <span className="text-xs text-[var(--color-muted)]">اكتمال التسعير: {compliance.completionPct}%</span>
                )}
              </div>
            </div>
          )}
          ...
          <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4 md:p-5 shadow-sm">
            ...
            <div className="flex flex-col gap-2">
              {(topBuckets || []).length === 0 ? (
                ...
                topBuckets.map(b => (
                  <div key={b.bucket} className="text-sm text-[var(--color-text)] flex items-center justify-between gap-2 py-1 border-b border-[var(--color-border)] last:border-0">
