...
export function computeCommissionTotals(cms) {
  ...
  const pendingOffice = pending.reduce(
    (s, c) => s + ((Number(c.dealValue) || 0) * (Number(c.officePercent) || 0)) / 100,
    0
  );
  const paidOffice = paid.reduce(
    (s, c) => s + ((Number(c.dealValue) || 0) * (Number(c.officePercent) || 0)) / 100,
    0
  );
  const totalAgent = list.reduce(
    (s, c) => s + ((Number(c.dealValue) || 0) * (Number(c.agentPercent) || 0)) / 100,
    0
  );
