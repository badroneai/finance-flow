    ...
    <header className="bg-[var(--color-surface)] border-b border-[var(--color-border)] sticky top-0 z-30 no-print flex flex-col">
      ...
      <div className="px-4 py-3 flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <h2 className="text-lg font-bold text-[var(--color-text)] truncate">{titles[page] || ''}</h2>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <div className="text-xs text-[var(--color-muted)] text-start max-w-[14rem] sm:max-w-none whitespace-normal leading-snug" dir="auto" aria-label="التاريخ">
            {headerDateText || ''}
          </div>
        </div>
      </div>
