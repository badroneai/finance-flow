...
export const TrustChecks = () => {
  ...
  useEffect(() => {
    if (detectPrivateBrowsing()) {
      toast.warn(
        'البيانات تُحفظ على هذا الجهاز فقط. في وضع التصفح الخاص قد تُفقد عند إغلاق المتصفح. للحفاظ عليها استخدم التصفح العادي.'
      );
    }
    if (!checkStorageQuota(storageFacade)) {
      toast.warn(
        'مساحة التخزين قريبة من الامتلاء. يُنصح بتصدير نسخة احتياطية أو حذف بعض البيانات.'
      );
    }
  }, [toast]);
  return null;
