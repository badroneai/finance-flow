...
export function DataProvider({ children }) {
  ...
  const fetchTransactions = useCallback(async (filters = {}) => {
    ...
    try {
      if (isCloud) {
        ...
        if (filters.startDate || filters.fromDate) sbFilters.startDate = filters.startDate || filters.fromDate;
        if (filters.endDate || filters.toDate) sbFilters.endDate = filters.endDate || filters.toDate;
  ...
  const fetchRecurringItems = useCallback(async (ledgerId = null) => {
  ...
  const createTransaction = useCallback(async (tx) => {
    try {
      if (isCloud) {
        ...
        if (!result || !result.ok) return { data: null, error: { message: result?.message || 'فشل الحفظ' } };
        await fetchTransactions();
  ...
  const updateTransaction = useCallback(async (id, updates) => {
    try {
      if (isCloud) {
        ...
        if (!result || !result.ok) return { data: null, error: { message: result?.message || 'فشل التحديث' } };
        await fetchTransactions();
        return { data: result.item, error: null };
      }
    } catch (err) {
  ...
  const createLedger = useCallback(async (ledger) => {
    try {
      if (isCloud) {
        const payload = toSnakeCase({
          ...ledger,
  ...
  const updateLedger = useCallback(async (id, updates) => {
    try {
      if (isCloud) {
        const payload = toSnakeCase(updates);
        delete payload.id;
  ...
  const createRecurringItem = useCallback(async (item) => {
    try {
      if (isCloud) {
        const payload = toSnakeCase({
          ...item,
  ...
  const updateRecurringItem = useCallback(async (id, updates) => {
    try {
      if (isCloud) {
        const payload = toSnakeCase(updates);
        delete payload.id;
  ...
  const deleteRecurringItem = useCallback(async (id) => {
    try {
      if (isCloud) {
  ...
  const createCommission = useCallback(async (commission) => {
    try {
      if (isCloud) {
        const payload = toSnakeCase({
          ...commission,
  ...
  const updateOfficeSettings = useCallback(async (settings) => {
    try {
      if (isCloud) {
  ...
  const setActiveLedgerId = useCallback((id) => {
    ...
    if (!isCloud) {
      try { setActiveLedgerIdLocal(id); } catch {}
    }
  }, [isCloud]);
  ...
  useEffect(() => {
    ...
    fetchRecurringItems();
    fetchCommissions();
  }, [officeId, fetchLedgers, fetchTransactions, fetchRecurringItems, fetchCommissions]);
  ...
  const value = useMemo(() => ({
    ...
    transactions, recurringItems, commissions,
    ledgersLoading, transactionsLoading, recurringLoading, commissionsLoading,
    fetchLedgers, fetchTransactions, fetchRecurringItems, fetchCommissions,
    createTransaction, updateTransaction, deleteTransaction,
    createLedger, updateLedger, deleteLedger,
    createRecurringItem, updateRecurringItem, deleteRecurringItem,
    createCommission, updateCommission, deleteCommission,
    updateOfficeSettings, isCloud,
  ]);
