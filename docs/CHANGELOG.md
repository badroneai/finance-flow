# سجل التغييرات — قيد العقار

## [بدون إصدار] – إصلاحات موبايل (الدفعة 6)

- **الهدف**: إضافة animation خفيف واحترام تفضيل تقليل الحركة (أولوية منخفضة — animations + إمكانية وصول).
- **Animation**: عند فتح «فلاتر متقدمة» في سجل العمليات أو العمولات، ظهور اللوحة مع fade-in 0.2s — يُطبَّق فقط عندما يكون تفضيل النظام prefers-reduced-motion: no-preference.
- **تقليل الحركة**: عند prefers-reduced-motion: reduce تُلغى مدة كل الـ animations والـ transitions (0.01ms) لتفادي الدوخة أو الانزعاج.
- **الملفات المُعدَّلة**: `finance-flow.html`، `docs/mobile-fixes.md`، `docs/implementation-guide.md`، `docs/CHANGELOG.md`.

---

## [بدون إصدار] – إصلاحات موبايل (الدفعة 5)

- **الهدف**: تحسين scrollbar على الموبايل وتحسين هوامش الطباعة (أولوية منخفضة).
- **Scrollbar (موبايل)**: عرض وارتفاع 8px (بدل 6px) لسهولة اللمس؛ border-radius للمقبض؛ لون أغمق عند الحالة النشطة (active).
- **الطباعة**: إلغاء padding الجسم (left, right, top) وpadding الـ print-container في وضع الطباعة لتفادي تأثير safe-area على الهوامش المطبوعة.
- **الملفات المُعدَّلة**: `finance-flow.html`، `docs/mobile-fixes.md`، `docs/implementation-guide.md`، `docs/CHANGELOG.md`.

---

## [بدون إصدار] – إصلاحات موبايل (الدفعة 4)

- **الهدف**: تطبيق الدفعة الرابعة: Touch targets 48px، عناوين متجاوبة clamp، تحسينات طباعة.
- **Touch targets (موبايل فقط)**: حد أدنى 48px ارتفاعًا لجميع الأزرار؛ عناصر القائمة الجانبية 48px مع padding مناسب؛ أزرار تعديل/حذف في الجداول 48×48px مع padding 12px.
- **عناوين رئيسية clamp**: عنوان الهيدر (h2) بحجم `clamp(0.95rem, 2.5vw, 1.125rem)`؛ نص القيمة الرئيسية في الصفحة الرئيسية `clamp(1.15rem, 4vw, 1.8rem)`؛ عنوان «قيد العقار» في الصفحة الرئيسية `clamp(1.25rem, 5vw, 1.875rem)`.
- **الطباعة**: إلغاء min-height/min-width للأزرار في وضع الطباعة لتفادي تشوه التخطيط.
- **الملفات المُعدَّلة**: `finance-flow.html`، `docs/mobile-fixes.md`، `docs/implementation-guide.md`، `docs/CHANGELOG.md`.

---

## [بدون إصدار] – إصلاحات موبايل (الدفعة 3)

- **الهدف**: تطبيق الدفعة الثالثة: فلاتر قابلة للطي، بطاقة عمولات مدفوعة، قوالب خطابات مضغوطة.
- **تحليل الأداء**: البطاقة الخامسة «عمولات مدفوعة» داخل غلاف `col-span-2 md:col-span-1` لتصبح بعرض كامل على الموبايل وتجنب بقائها وحدها في صف.
- **سجل العمليات والعمولات**: بحث ظاهر دائمًا؛ زر «فلاتر متقدمة» (ظاهر على الموبايل فقط) يفتح/يغلق صف الفلاتر (نوع، تصنيف، تواريخ، إلخ). على الشاشات المتوسطة وما فوق الفلاتر ظاهرة دائمًا.
- **قوالب الخطابات**: بطاقات بتصميم أفقي (أيقونة يسار، عنوان ووصف وزر يمين)، تقليل padding، زر «استخدام القالب» غير بعرض كامل؛ شبكة sm:grid-cols-2 lg:grid-cols-3.
- **الملفات المُعدَّلة**: `finance-flow.html`، `docs/mobile-fixes.md`، `docs/implementation-guide.md`، `docs/CHANGELOG.md`.

---

## [بدون إصدار] – إصلاحات موبايل (الدفعة 2)

- **الهدف**: تطبيق الدفعة الثانية من تحسينات الموبايل (بطاقات الملخص 2+1، جداول مع تمرير أفقي).
- **سجل العمليات**: بطاقات الملخص من 3 في صف واحد إلى شبكة 2+1 (إجمالي الدخل، إجمالي الخرج في صف؛ الصافي بعرض كامل col-span-2). جدول المعاملات: غلاف relative + overflow-x-auto، جدول min-w-[600px]، ونص تلميح «← اسحب لعرض المزيد →» يظهر على الموبايل فقط (md:hidden).
- **العمولات**: نفس تحسين الجدول (min-w-[600px]، overflow-x-auto، تلميح اسحب).
- **الملفات المُعدَّلة**: `finance-flow.html`، `docs/CHANGELOG.md`، `docs/mobile-fixes.md`، `docs/implementation-guide.md`.

---

## [بدون إصدار] – إصلاحات موبايل (الدفعة 1) + آلية العمل

- **الهدف**: تطبيق تحسينات الموبايل على دفعات مع توثيق آلية العمل وعدم تنفيذ كل التحسينات دفعة واحدة.
- **آلية العمل**: تم توثيق آلية في `docs/implementation-guide.md` و`docs/mobile-fixes.md`: تنفيذ دفعة → اختبار → توثيق → دفعة تالية. مرجع الحلول: `assets/mobile-responsive-fixes.css`.
- **الدفعة 1 (مُنفَّذة)**:
  - **finance-flow.html**: تحديث viewport إلى `maximum-scale=5, viewport-fit=cover`؛ إضافة أنماط safe-area (env(safe-area-inset-*)) للهيدر والجسم؛ منع زوم iOS على الحقول (font-size 16px على الشاشات ≤768px)؛ لمس (opacity على :active للأزرار والروابط).
- **الملفات المُعدَّلة**: `finance-flow.html`، `docs/implementation-guide.md`، `docs/mobile-fixes.md`، `docs/CHANGELOG.md`.

---

## v0.7.3 – Phase 10.3.1 Wire landing CTAs to product

- **الهدف**: ربط أزرار صفحة البيع بالمنتج وقنوات التواصل دون تعديل finance-flow.html.
- **landing.html**: زر «شاهد الديمو» أصبح يفتح `./finance-flow.html` في تبويب جديد (`target="_blank"`). إضافة سطر نص تحت الأزرار: «ملاحظة: الديمو يعمل محليًا داخل المتصفح، والبيانات تُحفظ على جهازك.» وإضافة رابط نصي صغير «شاهد لقطات الشاشة» إلى `#demo`. في قسم التواصل استبدال placeholders بروابط قابلة للعمل (wa.me، mailto، t.me) مع نصوص «ضع رقمك هنا» / «ضع إيميلك هنا» / «ضع معرفك هنا» لاستبدالها لاحقاً بالبيانات الحقيقية.
- الملفات المُعدّلة: `landing.html`، `docs/CHANGELOG.md`.

---

## v0.7.2 – Phase 10.3 Static landing page (landing.html)

- **الهدف**: صفحة بيع ثابتة (Static HTML/CSS) دون أي تعديل على التطبيق أو الكود الداخلي.
- **landing.html**: صفحة واحدة RTL عربية تشمل: Hero (عنوان، وصف، 3 نقاط قيمة، CTA شاهد الديمو / اطلب النسخة)، لمن المنتج (مناسب/غير مناسب)، ماذا يقدم (6 بطاقات ميزات)، لقطات placeholder (4 مربعات)، الباقات (Basic 399 / Pro 899 ⭐ / Office 1,499) مع جدول مقارنة وملاحظة الترخيص، FAQ (6 أسئلة)، نموذج تواصل (الاسم، المكتب، الجوال، الباقة، زر إرسال مع alert تجريبي)، قنوات placeholder (واتساب، إيميل، تيليجرام)، Footer مع تنبيه البيانات محلية. تصميم بألوان مشابهة للمشروع، responsive، بدون React ولا مكتبات جديدة.
- الملفات المُعدّلة: `landing.html` (جديد)، `docs/CHANGELOG.md`.

---

## v0.7.1 – Phase 10.2 Pricing & Packaging

- **الهدف**: توثيق التسعير والتغليف دون أي تعديل على الكود أو الواجهة.
- **docs/PRICING.md**: منطق التسعير (الشريحة المستهدفة، طبيعة الاستخدام، لماذا ليس SaaS ولا اشتراك إجباري)، ثلاث باقات (Basic 399 ر.س، Pro 899 ر.س موصى بها، Office 1,499 ر.س) مع التمايز في الدعم والتخصيص والتسليم والإعداد والتحديثات فقط — بدون ميزات غير موجودة في المنتج، جدول مقارنة الباقات، سياسة البيع (ترخيص مرة واحدة، جهاز/مكتب، تحديثات، دعم)، وخمس اعتراضات سعر مع ردود مختصرة.
- الملفات المُعدّلة: `docs/PRICING.md` (جديد)، `docs/CHANGELOG.md`.

---

## v0.7.0 – Phase 10.1 Demo Script & Sales Narrative

- **الهدف**: توثيق العرض البيعي والسرد الموجه للمبيعات دون أي تغيير في الكود أو المنتج.
- **docs/DEMO_SCRIPT.md**: سكربت عرض 7–10 دقائق يشمل: الافتتاح (المشكلة، فشل Excel/الدفاتر/الواتساب، Hook)، الحل (ما هو قيد العقار ولماذا مختلف)، العرض المباشر (لوحة المعلومات، حركة مالية، تصفية/بحث/CSV، العمولات، الخطابات، الإعدادات) بصيغة «ماذا أقول + ماذا أفتح»، الأمان والخصوصية، الإغلاق (لمن المنتج، لمن ليس، الخطوة التالية).
- **docs/SALES_NARRATIVE.md**: Value Proposition (ثلاث نقاط: وفر وقت، قلل أخطاء، سيطرة على البيانات)، ست اعتراضات شائعة مع الردود، جدول مقارنة قيد العقار مقابل الطرق التقليدية، Elevator Pitch (30 ثانية). أسلوب عربي واضح موجّه لمكتب عقاري صغير سعودي.
- الملفات المُعدّلة: `docs/DEMO_SCRIPT.md` (جديد)، `docs/SALES_NARRATIVE.md` (جديد)، `docs/CHANGELOG.md`.
